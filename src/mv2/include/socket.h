...
public:
	...
	int Bind(const char* hostname, const char* port, unsigned int portNum);
	int Receive(void* out_buf, size_t count);
	void SetNonBlocking();
	int Read(unsigned char* buf, uint size);
...
class SocketServerUDP {
  public:
    static const int kReceiveBufferSize = 20 * 1024;

    SocketServerUDP(SocketEventHandlerInterface* handler,
                 const char* port,
  ...
  private:
    ...
    bool HandleAccept();
    bool HandleRead(UDPSocket* sock);
    bool HandleClose(UDPSocket* sock);
    void ShutdownClient(int sock_fd);

    UDPSocket server_sock_;
    std::map< int, std::tr1::shared_ptr<UDPSocket> > client_socks_;
    ...
    DISALLOW_COPY_AND_ASSIGN(SocketServerUDP);
};
...
public:
    ...
    SocketClientTCP(SocketEventHandlerInterface* handler, unsigned int size);
